...
どのスタックでも共通の原則と、Next.js + Prisma(デフォルト構成)での具体形を示す。
別スタックの場合は原則の方を優先し、具体形(prisma コマンド、Dockerfile、
デプロイ先ごとの手順)は選定したスタック・デプロイ先の慣習に読み替える。
デプロイ先の節は、このプロジェクトで選定した環境のものだけを読めばよい。
