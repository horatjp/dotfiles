...
出目は起点であって命令ではない。振り直しは自由だが、
振るたびに理由を1行ブリーフに残す(SKILL.md手順4)。
"""
import argparse
import pathlib
import random
...
def main() -> int:
    ...
    print(f"母集団: 骨格{len(S)} ヒーロー{len(H)} 制約{len(C)}(除外{len(excl)}件)"
          "——カタログ編集後に数が不自然に減っていたら書式崩れを疑う")
    for i in range(n):
        print(f"案{i + 1}: 骨格 {skeletons[i][0]} {skeletons[i][1]}"
              f" / ヒーロー {heroes[i][0]} {heroes[i][1]}"
              f" / 制約 {constraints[i][0]} {constraints[i][1]}")
    print("※接続テスト(戦略との繋がりを1文)を通った出目だけ採用。振り直しは自由、ただし振るたびに理由を1行残すこと。")
    return 0
